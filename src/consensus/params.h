 ...
 *
 * Tcoin: Blossom activates at height 1 on mainnet and testnet, so the
 * post-Blossom value is the block time that actually applies: 75 seconds.
 * The pre-Blossom value only governs the genesis block and is kept at the
 * Zcash value because it must be an exact multiple of the post-Blossom one.
 */
...
static const unsigned int PRE_BLOSSOM_HALVING_INTERVAL = 50000000;
